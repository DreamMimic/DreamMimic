...
class InterMimicG1(Humanoid_G1, InterMimic):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        # 在super().__init__()之前初始化标志，因为_create_envs可能在super().__init__()中被调用
        self._key_body_indices_initialized = False
        ...
        # 注意：body_names_to_idx 在 _create_envs 中初始化，所以这里先不初始化
        # 将在 _create_envs 之后调用 _init_key_body_indices
        # _key_body_indices_initialized 已在 __init__ 最开始设置
        return
    
    def _init_key_body_indices(self):
        """初始化用于reward计算的关键body索引"""
        # 检查 body_names_to_idx 是否已初始化
        if not hasattr(self, 'body_names_to_idx') or self.body_names_to_idx is None:
            return
        
        # G1 body索引（从body_names_to_idx获取）
        self._left_ankle_idx = self.body_names_to_idx.get('left_ankle_roll_link', None)
        self._right_ankle_idx = self.body_names_to_idx.get('right_ankle_roll_link', None)
        self._left_palm_idx = self.body_names_to_idx.get('left_hand_palm_link', None)
        self._right_palm_idx = self.body_names_to_idx.get('right_hand_palm_link', None)
        
        # SMPLX body索引（root/pelvis是0，L_Wrist和R_Wrist在keyIndex中对应17和36）
        # 根据keyIndex映射：keyIndex[10]=17对应left_wrist_yaw_link，keyIndex[16]=36对应right_wrist_yaw_link
        # 但SMPLX中L_Wrist和R_Wrist的52槽位索引是17和36
        # 注意：SMPLX的body_pos是52维，索引0是root/pelvis
        self._smplx_pelvis_idx = 0  # root/pelvis在52槽位中的索引
        self._smplx_left_wrist_idx = 17  # L_Wrist在52槽位中的索引
        self._smplx_right_wrist_idx = 36  # R_Wrist在52槽位中的索引
        
        # 检查G1的ankle是否在keyBodies中
        try:
            if 'left_ankle_roll_link' in self.key_bodies:
                self._left_ankle_key_idx = self.key_bodies.index('left_ankle_roll_link')
            else:
                self._left_ankle_key_idx = None
            if 'right_ankle_roll_link' in self.key_bodies:
                self._right_ankle_key_idx = self.key_bodies.index('right_ankle_roll_link')
            else:
                self._right_ankle_key_idx = None
        except Exception:
            self._left_ankle_key_idx = None
            self._right_ankle_key_idx = None
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # 在 _create_envs 之后初始化关键body索引（此时 body_names_to_idx 已可用）
        # 使用 hasattr 检查以确保安全
        if not hasattr(self, '_key_body_indices_initialized') or not self._key_body_indices_initialized:
            self._init_key_body_indices()
            self._key_body_indices_initialized = True
        return
    ...
    def _init_retarget_mapping(self):
        # Build name -> local index mapping for selected key bodies in tensors (N, K, 3)
        ...
        # Define simple limb chains (parent -> child) using available G1 link names
    ...
    def _retarget_ref_keypos(self, ref_key_pos_52, curr_key_pos_52):
        # ref_key_pos_52: (N, 52, 3) reference (SMPLX) in GT layout
        # curr_key_pos_52: (N, 52, 3) current robot body positions in GT layout
        self._init_retarget_mapping()
        ret = ref_key_pos_52.clone()
        eps = 1e-6
        for parent_name, child_name in self._retarget_chains:
            p_slot = self._gt_slot_by_name.get(parent_name, None)
            c_slot = self._gt_slot_by_name.get(child_name, None)
            if p_slot is None or c_slot is None:
                continue
            # segment vectors
            vh = ref_key_pos_52[:, c_slot, :] - ref_key_pos_52[:, p_slot, :]
            vr = curr_key_pos_52[:, c_slot, :] - curr_key_pos_52[:, p_slot, :]
            # length-based scale
            s = (vr.norm(dim=-1, keepdim=True) + eps) / (vh.norm(dim=-1, keepdim=True) + eps)
            # apply scaling along the chain in world frame
            ret[:, c_slot, :] = ret[:, p_slot, :] + s * vh
        return ret

    def compute_humanoid_reward_g1(self, w):
        """
        G1-specific humanoid reward computation.
        只关注关键部位，避免retarget误差误导：
        - 脚底（left_ankle_roll_link和right_ankle_roll_link）的3D位置
        - pelvis（root）的xy位置（不含z）
        - 左右手palm（left_hand_palm_link和right_hand_palm_link）的xy位置（不含z）
        """
        # 确保关键body索引已初始化
        if not hasattr(self, '_key_body_indices_initialized') or not self._key_body_indices_initialized:
            self._init_key_body_indices()
        ...
        # 从52槽位的body_pos中获取关键部位位置（用于ankle，因为它们已在keyBodies中）
        body_pos_full = self.extract_data_component('body_pos', obs=self._curr_obs).view(self._curr_obs.shape[0], -1, 3)  # (N, 52, 3)
        ref_body_pos_full = self.extract_data_component('body_pos', obs=self._curr_ref_obs).view(self._curr_ref_obs.shape[0], -1, 3)  # (N, 52, 3)
        
        # 获取G1当前body位置（从rigid_body_pos获取，用于palm，因为它们不在keyBodies中）
        curr_body_pos = self._rigid_body_pos  # (N, num_bodies, 3)
        
        # 收集关键部位的位置
        key_positions = []
        ref_key_positions = []
        
        # 1. 脚底：left_ankle_roll_link和right_ankle_roll_link的3D位置
        # ankle在keyBodies中，所以从52槽位的body_pos中获取
        if hasattr(self, '_left_ankle_key_idx') and self._left_ankle_key_idx is not None:
            # G1的left_ankle在52槽位中的位置（通过_key_body_ids_gt映射）
            g1_left_ankle_52_idx = self._key_body_ids_gt[self._left_ankle_key_idx]  # 在52槽位中的索引
            left_ankle_pos = body_pos_full[:, g1_left_ankle_52_idx, :]  # (N, 3)
            # SMPLX中L_Ankle在52槽位中索引是3
            smplx_left_ankle_idx = 3  # L_Ankle在SMPLX 52槽位中的索引
            ref_left_ankle_pos = ref_body_pos_full[:, smplx_left_ankle_idx, :]  # (N, 3)
            key_positions.append(left_ankle_pos)
            ref_key_positions.append(ref_left_ankle_pos)
        
        if hasattr(self, '_right_ankle_key_idx') and self._right_ankle_key_idx is not None:
            # G1的right_ankle在52槽位中的位置
            g1_right_ankle_52_idx = self._key_body_ids_gt[self._right_ankle_key_idx]  # 在52槽位中的索引
            right_ankle_pos = body_pos_full[:, g1_right_ankle_52_idx, :]  # (N, 3)
            # SMPLX中R_Ankle在52槽位中索引是7
            smplx_right_ankle_idx = 7  # R_Ankle在SMPLX 52槽位中的索引
            ref_right_ankle_pos = ref_body_pos_full[:, smplx_right_ankle_idx, :]  # (N, 3)
            key_positions.append(right_ankle_pos)
            ref_key_positions.append(ref_right_ankle_pos)
        
        # 2. pelvis（root）的xy位置（不含z）
        root_pos = self.extract_data_component('root_pos', obs=self._curr_obs)  # (N, 3)
        ref_root_pos = self.extract_data_component('root_pos', obs=self._curr_ref_obs)  # (N, 3)
        pelvis_xy = root_pos[:, :2]  # (N, 2) 只取xy
        ref_pelvis_xy = ref_root_pos[:, :2]  # (N, 2)
        key_positions.append(pelvis_xy)
        ref_key_positions.append(ref_pelvis_xy)
        
        # 3. 左右手palm的xy位置（不含z）
        # palm不在keyBodies中，所以直接从rigid_body_pos获取
        if hasattr(self, '_left_palm_idx') and self._left_palm_idx is not None:
            left_palm_pos = curr_body_pos[:, self._left_palm_idx, :]  # (N, 3)
            left_palm_xy = left_palm_pos[:, :2]  # (N, 2) 只取xy
            # SMPLX中L_Wrist在52槽位中索引是17
            ref_left_wrist_pos = ref_body_pos_full[:, self._smplx_left_wrist_idx, :]  # (N, 3)
            ref_left_wrist_xy = ref_left_wrist_pos[:, :2]  # (N, 2)
            key_positions.append(left_palm_xy)
            ref_key_positions.append(ref_left_wrist_xy)
        
        if hasattr(self, '_right_palm_idx') and self._right_palm_idx is not None:
            right_palm_pos = curr_body_pos[:, self._right_palm_idx, :]  # (N, 3)
            right_palm_xy = right_palm_pos[:, :2]  # (N, 2) 只取xy
            # SMPLX中R_Wrist在52槽位中索引是36
            ref_right_wrist_pos = ref_body_pos_full[:, self._smplx_right_wrist_idx, :]  # (N, 3)
            ref_right_wrist_xy = ref_right_wrist_pos[:, :2]  # (N, 2)
            key_positions.append(right_palm_xy)
            ref_key_positions.append(ref_right_wrist_xy)
        
        # 如果没有找到任何关键部位，返回默认reward
        if len(key_positions) == 0:
            rb = torch.ones(self.num_envs, device=self.device)
            human_reset = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
            return rb, human_reset, None, None
        
        # 拼接所有关键部位位置
        key_pos = torch.cat(key_positions, dim=-1)  # (N, total_dim)
        ref_key_pos = torch.cat(ref_key_positions, dim=-1)  # (N, total_dim)
        
        # 计算位置误差（所有关键部位）
        pos_diff = (ref_key_pos - key_pos) ** 2  # (N, total_dim)
        ep = pos_diff.mean(dim=-1)  # (N,)
        rp = torch.exp(-ep * w['p'])
        
        # Body rotation reward（保持原有逻辑，但权重可以降低）
        ...
        # 只对关键部位计算旋转误差（可选，如果不需要可以设为1）
        er = torch.mean(diff[:, :] * 0.1, dim=-1)  # 降低旋转权重
        rr = torch.exp(-er * w['r'])
        
        # Body velocity rewards（保持原有逻辑）
        ...
        # 返回用于后续IG reward计算的key_pos（使用52槽位格式）
        key_pos_for_ig = body_pos_full[:, self._key_body_ids_gt] if hasattr(self, '_key_body_ids_gt') else None
        ref_key_pos_for_ig = ref_body_pos_full[:, self._key_body_ids_gt] if hasattr(self, '_key_body_ids_gt') else None
        
        return rb, human_reset, key_pos_for_ig, ref_key_pos_for_ig
