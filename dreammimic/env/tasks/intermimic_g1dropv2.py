...
class InterMimicG1(Humanoid_G1, InterMimic):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.vis_env_id = cfg['env'].get('visualizeEnvId', 0)  # 要可视化的环境ID
    ...
    def _visualize_link_mapping(self, key_pos, ref_key_pos):
        """
        可视化参考link和实际link的位置映射关系
        
        可视化方式：
        1. 参考位置：用RGB颜色编码XYZ坐标的坐标轴（红色=X，绿色=Y，蓝色=Z）
        2. 实际位置：用RGB颜色编码XYZ坐标的坐标轴
        3. 连接线：从参考位置到实际位置，颜色表示误差大小
           - 红色=大误差（>0.3m）
           - 黄色=中等误差（0.1-0.3m）
           - 绿色=小误差（<0.1m）
        4. 使用较长的坐标轴线段（0.1m）使其更显眼
        """
        if key_pos is None or ref_key_pos is None:
            return
        ...
        # 获取当前环境的参考和实际key位置
        # key_pos: (N, K, 3) where K is len(self._key_body_ids_gt)
        # ref_key_pos: (N, K, 3)
        curr_key_pos = key_pos[env_id].cpu().numpy()  # (K, 3)
        curr_ref_key_pos = ref_key_pos[env_id].cpu().numpy()  # (K, 3)
        ...
        # 准备线条数据
        num_links = curr_key_pos.shape[0]
        lines = []
        colors = []
        ...
        for i in range(num_links):
            ref_pos = curr_ref_key_pos[i]  # (3,)
            curr_pos = curr_key_pos[i]  # (3,)
            
            # 计算误差
            error = np.linalg.norm(ref_pos - curr_pos)
            ...
            # 添加从参考位置到实际位置的连接线（更粗的视觉效果：重复绘制多次）
            for _ in range(3):  # 重复3次使线条更显眼
                lines.append([ref_pos[0], ref_pos[1], ref_pos[2], 
            ...
            for _ in range(2):
                lines.append([ref_pos[0], ref_pos[1], ref_pos[2],
                             ref_pos[0] + axis_length, ref_pos[1], ref_pos[2]])
                colors.append([1.0, 0.0, 0.0])  # 红色
            # Y轴（绿色）
            for _ in range(2):
                lines.append([ref_pos[0], ref_pos[1], ref_pos[2],
                             ref_pos[0], ref_pos[1] + axis_length, ref_pos[2]])
                colors.append([0.0, 1.0, 0.0])  # 绿色
            # Z轴（蓝色）
            for _ in range(2):
                lines.append([ref_pos[0], ref_pos[1], ref_pos[2],
                             ref_pos[0], ref_pos[1], ref_pos[2] + axis_length])
                colors.append([0.0, 0.0, 1.0])  # 蓝色
            
            # 添加实际位置的坐标轴（RGB编码XYZ）
            # X轴（红色）
            for _ in range(2):
                lines.append([curr_pos[0], curr_pos[1], curr_pos[2],
                             curr_pos[0] + axis_length, curr_pos[1], curr_pos[2]])
                colors.append([1.0, 0.0, 0.0])  # 红色
            # Y轴（绿色）
            for _ in range(2):
                lines.append([curr_pos[0], curr_pos[1], curr_pos[2],
                             curr_pos[0], curr_pos[1] + axis_length, curr_pos[2]])
                colors.append([0.0, 1.0, 0.0])  # 绿色
            # Z轴（蓝色）
            for _ in range(2):
                lines.append([curr_pos[0], curr_pos[1], curr_pos[2],
                             curr_pos[0], curr_pos[1], curr_pos[2] + axis_length])
                colors.append([0.0, 0.0, 1.0])  # 蓝色
