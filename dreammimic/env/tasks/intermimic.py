...
class InterMimic(Humanoid_SMPLX):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.print_reset_motion = bool(cfg['env'].get('printResetMotion', False))
        ...
        self.reward_stats = {}
        self._use_student_reward_curriculum = bool(cfg["env"].get("useStudentRewardCurriculum", False))
        self._curr_is_student_rollout = torch.ones([self.num_envs], device=self.device, dtype=torch.bool)
    ...
    def _load_target_asset(self): # smplx
        asset_root = "dreammimic/data/assets/objects/"
        self._target_asset = []
        points_num = []
        self.object_points = []
        for i, object_name in enumerate(self.object_name):
            ...
            object_points, object_faces = trimesh.sample.sample_surface_even(mesh_obj, count=512, seed=2024)

            object_points = to_torch(object_points - center)
            

            while object_points.shape[0] < 512:
                object_points = torch.cat([object_points, object_points[:512 - object_points.shape[0]]], dim=0)
    ...
    def _build_target(self, env_id, env_ptr):
        col_group = env_id
        col_filter = 0
        # Assign different segmentation IDs to different object types
        # Object ID starts from 2 (0=ground, 1=humanoid, 2+=objects)
        object_type_id = env_id % len(self.object_name)
        segmentation_id = 2 + object_type_id

        default_pose = gymapi.Transform()
        
        target_handle = self.gym.create_actor(env_ptr, self._target_asset[object_type_id], default_pose, self.object_name[object_type_id], col_group, col_filter, segmentation_id)
    ...
    def _maybe_print_reset_motions(self, env_ids, motion_ids, ref_indices, motion_times):
        if not self.print_reset_motion:
            return

        env_ids_cpu = env_ids.detach().cpu().tolist()
        motion_ids_cpu = motion_ids.detach().cpu().tolist()
        ref_indices_cpu = ref_indices.detach().cpu().tolist()
        motion_times_cpu = motion_times.detach().cpu().tolist()
        for env_id, motion_id, ref_index, motion_time in zip(env_ids_cpu, motion_ids_cpu, ref_indices_cpu, motion_times_cpu):
            motion_name = os.path.basename(self.motion_file[motion_id])
            dataset_id = int(self.dataset_index[motion_id].detach().cpu().item())
            print(
                f"[ResetMotion] env={env_id} dataset_id={dataset_id} "
                f"motion={motion_name} ref_index={ref_index} start={motion_time}",
                flush=True,
            )
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        self._maybe_print_reset_motions(env_ids, i, idx, motion_times)
    ...
    def _compute_observations(self, env_ids=None):
        if (env_ids is None):
            self._curr_ref_obs[:] = self.hoi_data[self.data_id, self.progress_buf].clone()
            obs_1 = self._compute_observations_iter(self.hoi_data, None, 1)
            obs_16 = self._compute_observations_iter(self.hoi_data, None, 16)
            self.obs_buf[:] = torch.cat((obs_1, obs_16), dim=-1)
    ...
    def _compute_reset(self):
        ...
        if self.enable_evaluation:
            ...
            # Also update metrics for non-reset environments (running sequences)
            # This ensures we track progress even if sequences don't complete
            if len(reset_id) == 0:
                # No resets, but update running sequences
                for env_id in range(self.num_envs):
                    seq_id = self.data_id[env_id]
                    curr_steps = self.progress_buf[env_id]
                    if curr_steps > 0 and self._max_execution_steps[seq_id] < curr_steps:
                        self._max_execution_steps[seq_id] = curr_steps
                        if curr_steps > 1:
                            self._best_human_pose_error_per_seq[seq_id] = self._human_pose_error_per_seq_step[seq_id, 1:curr_steps].mean()
                            self._best_object_pose_error_per_seq[seq_id] = self._object_pose_error_per_seq_step[seq_id, 1:curr_steps].mean()

            if (self._max_execution_steps >= 1).any() and flag:
                ...
                success_threshold = self._get_eval_success_threshold()
                success_count = torch.sum(self._max_execution_steps - success_threshold >= 0)
        ...
        if self.reset_buf.sum() > 0 and self.psi > 1:
            if self.ref_reward.shape[1] <= 1 or self.hoi_refs.shape[1] <= 1:
                if not hasattr(self, "_physical_buffer_warned"):
                    print(
                        f"Warning: physicalBufferSize={self.psi}, but loaded reference buffers "
                        f"have shape {tuple(self.ref_reward.shape)}. Skipping physical-buffer curriculum."
                    )
                    self._physical_buffer_warned = True
                return
            reset_ind = (self.reset_buf == 1)
            if self._use_student_reward_curriculum:
                reset_ind = torch.logical_and(reset_ind, self._curr_is_student_rollout)
                if reset_ind.sum() == 0:
                    return
    ...
    def _update_reward_stats(self, rb, ro, rig, rcg, final_reward):
        try:
            self.reward_stats = {
                "rb": float(rb.mean().item()),
                "ro": float(ro.mean().item()),
                "rig": float(rig.mean().item()),
                "rcg": float(rcg.mean().item()),
                "final": float(final_reward.mean().item()),
            }
        except Exception:
            # In case tensors contain NaNs during debugging, fall back to empty stats to avoid breaking logging.
            self.reward_stats = {}
    ...
    def render(self, sync_frame_time=False, t=0):
        ...
        if self.viewer:  
            if self.save_images:
                ...
                rgb_filename = "dreammimic/data/images/" + dataname + "/rgb_env%d_frame%05d.png" % (env_ids, frame_id)
                os.makedirs("dreammimic/data/images/" + dataname, exist_ok=True)
    ...
    def print_final_eval_summary(self):
        ...
        success_threshold = self._get_eval_success_threshold()[evaluated_mask]
        success_mask = self._max_execution_steps[evaluated_mask] - success_threshold >= 0
        success_count = torch.sum(success_mask)
        ...
        evaluated_indices = torch.nonzero(evaluated_mask, as_tuple=False).squeeze(-1)
        success_indices = evaluated_indices[success_mask]
        fail_indices = evaluated_indices[~success_mask]
        success_entries = [
            (os.path.basename(self.motion_file[idx]), int(self.max_episode_length[idx].item()))
            for idx in success_indices.tolist()
        ]
        fail_entries = [
            (os.path.basename(self.motion_file[idx]), int(self.max_episode_length[idx].item()))
            for idx in fail_indices.tolist()
        ]

        print("  Successful Trajectories:")
        if len(success_entries) > 0:
            for name, length in success_entries:
                print(f"    - {name} (length={length})")
        else:
            print("    - None")

        print("  Failed Trajectories:")
        if len(fail_entries) > 0:
            for name, length in fail_entries:
                print(f"    - {name} (length={length})")
        else:
            print("    - None")
        print("=" * 60 + "\n")

    def _get_eval_success_threshold(self):
        rollout_threshold = torch.full_like(self.max_episode_length, int(self.rollout_length) - 1)
        return torch.minimum(self.max_episode_length - 1, rollout_threshold)
