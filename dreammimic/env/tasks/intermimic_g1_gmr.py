...
class InterMimicG1(Humanoid_G1, InterMimic):
    ...
    def compute_humanoid_reward_g1(self, w):
        ...
        key_pos_full = self.extract_data_component('body_pos', obs=self._curr_obs).view(self._curr_obs.shape[0], -1, 3)#torch.Size([64, 52, 3])
        ref_key_pos_full = self.extract_data_component('body_pos', obs=self._curr_ref_obs).view(self._curr_ref_obs.shape[0], -1, 3)#torch.Size([64, 52, 3])
        key_pos = key_pos_full[:, self._key_body_ids_gt]#torch.Size([64, 20, 3])
        ref_key_pos = ref_key_pos_full[:, self._key_body_ids_gt]#torch.Size([64, 20, 3])  #tensor([ 1,  2,  3,  5,  6,  7,  9, 13, 15, 16, 17, 20, 23, 32, 34, 35, 36, 39,  42, 51], device='cuda:0')
        # Retarget reference key positions to robot limb scales (bone-aware scaling)
        ref_key_pos = self._retarget_ref_keypos(ref_key_pos, key_pos)
