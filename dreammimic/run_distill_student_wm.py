...
from env.tasks.intermimic_all_noref_vis_student_obs_wm import InterMimic_All_NoRef_Vis_StudentObs_WM
...
def main():
    ...
    cfg["env"]["recordVideo"] = bool(args.record_video)
    cfg["env"]["recordEnvIdx"] = int(args.record_env_idx)
    cfg["env"]["recordDir"] = args.record_dir or os.path.join(args.output_path, "record_video")
    cfg["env"]["recordEvery"] = max(1, int(args.record_every))
    cfg["env"]["recordMaxFrames"] = max(1, int(args.record_max_frames))
    cfg["env"]["recordFps"] = max(1, int(args.record_fps))
